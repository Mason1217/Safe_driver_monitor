...
class MonitorWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        info_layout.addWidget(self.lbl_alcohol_val, 1, 0)
        ...
        info_layout.addWidget(self.lbl_hrv_val, 1, 1)
        info_layout.addWidget(self.lbl_fatigue_status, 2, 1)
    ...
    def update_display(self, data_dict: dict):
        ...
        rr      = data_dict.get(DATA_DICT_KEY["rr"], 0)
        al_st   = data_dict.get(DATA_DICT_KEY["al_st"])
        f_st    = data_dict.get(DATA_DICT_KEY["f_st"])
        ...
        if f_st == FATIGUE_ST['F']:
            self.lbl_hrv_val.setStyleSheet("color: orange")
            self.lbl_fatigue_status.setStyleSheet("background-color: orange; color: white; padding: 5px;")
        else:
            self.lbl_hrv_val.setStyleSheet("color: black")
            self.lbl_fatigue_status.setStyleSheet("background-color: green; color: white; padding: 5px;")

        self.update_plot(rr)

    def update_plot(self, rr_val):
        '''
        Update rr plot
        ...
        cur_bpm = 0
        if rr_val > 0:
            cur_bpm = 60000 / rr_val
        
        self.y_data = self.y_data[1:]
        self.y_data.append(cur_bpm)
